...
parser.add_argument("--logdir", type=str, default=r"E:\code\logs\diedai10", help='path to model and log files')
parser.add_argument("--data_path", type=str, default=r"E:\code\datasets\Rain100L\rainy", help='path to training data')
parser.add_argument("--save_path", type=str, default=r"E:\code\datasets\Rain100L\rainy", help='path to save results')
...
def main():
    ...
    # Build model
    print('Loading model ...\n')
    model = PRDS(opt.recurrent_iter, opt.use_GPU)
    #model = PReNet_GRU(opt.recurrent_iter, opt.use_GPU)
    print_network(model)
    if opt.use_GPU:
        model = model.cuda()
